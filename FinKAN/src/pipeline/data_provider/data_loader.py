...
class PETR4(Dataset):
    def __init__(self, root_path : str, data_path : str,
                flag : str = 'train', scale : bool = True,
                freq : str ='15min', size : list = None):
        self.seq_len = size[0]
    ...
    def __read_data__(self):
        ...
        if self.scale:
            train_data_base = df_base[border1s[0] : border2s[0]]

            self.data_scaler.fit(train_data_base.values)
            data = self.data_scaler.transform(df_base.values)
        else:
            data = df_base.values
