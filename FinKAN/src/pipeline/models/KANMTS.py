...
class Mixer2dTriUKAN(nn.Module):
    ...
    def forward(self, inputs, *args, **kwargs): 
        ...
        combined_mean = self.net1(inputs)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.n_output_channels = self.c_out
        self.n_time_features = getattr(configs, 'n_time_features', 5)

        self.n_input_channels = self.n_output_channels + self.n_time_features 
        ...
        self.encoder = Encoder(
            [ 
                EncoderLayer(
                    Mixer2dTriUKAN(configs.d_model, self.n_input_channels, configs.d_core, configs.grid_size,
                                configs.hidden_dim,
                                configs.dropout),
                    configs.d_model,
                    configs.d_ff,
